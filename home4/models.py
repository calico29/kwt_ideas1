...
class Application(models.Model):
    ...
    application_id = models.UUIDField(
        unique=True,
        editable=True,  # Измените на True
        default=uuid.uuid4,
    ...
    authors = models.ManyToManyField(  # Изменено на ManyToMany
        User, 
        blank=True, 
        verbose_name="Авторы инновации",
    ...
    research_group = models.CharField(max_length=100, blank=True, null=True, verbose_name="Научная группа")
    pni_project = models.CharField(max_length=200, blank=True, null=True, verbose_name="Проект ПНИ")
    implementation_date = models.DateField(verbose_name="Дата внедрения", null=True, blank=True)
    development_basis = models.TextField(verbose_name="Основание для разработки", blank=True, null=True)
    customer = models.CharField(max_length=200, verbose_name="Заказчик", blank=True, null=True)
    description = models.TextField(verbose_name="Описание и характеристики разработки", blank=True, null=True)
    comparative_advantages = models.TextField(verbose_name="Сравнительные преимущества", blank=True, null=True)
    replication_assessment = models.TextField(verbose_name="Оценка возможности тиражирования", blank=True, null=True)
    ...
    def clean_readiness_level(self):
        data = self.cleaned_data['readiness_level']
        product_type = self.cleaned_data.get('scientific_product_type')
        if product_type == 'product_1' and int(data) < 4:
            raise ValidationError("Уровень готовности должен быть не менее 4 по ГОСТ Р 58048-2017")
        return data
