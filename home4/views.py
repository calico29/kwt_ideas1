...
from django.contrib import messages  # Добавить в начало файла
...
def change_username(request):
    context = {}
    session_data = request.session.get('username_change', {})
    remaining_cooldown = 0
    
    # Проверка кулдауна
    if session_data.get('last_sent'):
        try:
            last_sent = datetime.fromisoformat(session_data['last_sent'])
            cooldown = timedelta(seconds=60)
            remaining = (last_sent + cooldown) - timezone.now()
            remaining_cooldown = max(remaining.total_seconds(), 0)
    ...
    if request.method == "POST":
        if 'send_code' in request.POST:
            ...
            if User.objects.filter(username=new_username).exists():
                messages.error(request, "Этот юзернейм уже занят")
            else:
                ...
                request.session['username_change'] = session_data
            ...
            if code == session_data.get('code'):
                new_username = session_data.get('new_username')
                if not User.objects.filter(username=new_username).exists():
                    request.user.username = new_username
                    request.user.save()
                    del request.session['username_change']
                    messages.success(request, "Юзернейм успешно изменен!")
                    return redirect('profile')
                else:
                    messages.error(request, "Этот юзернейм стал занят в процессе смены")
            else:
...
def change_email(request):
    # Всегда получаем данные из сессии, даже для GET-запросов
    email_change_data = request.session.get('email_change') or {}
    remaining_cooldown = 0
    if email_change_data.get('last_sent'):
        try:
            last_sent = datetime.fromisoformat(email_change_data['last_sent'])
            cooldown = timedelta(seconds=60)
            remaining = (last_sent + cooldown) - timezone.now()
            remaining_cooldown = max(remaining.total_seconds(), 0)
        except (KeyError, ValueError):
            remaining_cooldown = 0
    # Проверка кулдауна
    if 'last_sent' in email_change_data:
        last_sent = datetime.fromisoformat(email_change_data['last_sent'])
        cooldown = timedelta(seconds=60)
        remaining = (last_sent + cooldown) - timezone.now()
        remaining_cooldown = max(remaining.total_seconds(), 0)
    if request.method == "POST":
        if 'send_code' in request.POST:
            ...
            # Генерируем код и сохраняем в сессии
            code = str(random.randint(100000, 999999))
            request.session['email_change'] = {
                'code': code,
                'new_email': new_email,
                'attempts': 3  # Лимит попыток
            }
            ...
            messages.info(request, "Код подтверждения отправлен на вашу текущую почту")
            ...
            user_code = request.POST.get('confirmation_code', '')
            
            if user_code == email_change_data['code']:
                # Меняем email
                request.user.email = email_change_data['new_email']
                request.user.save()
                ...
                email_change_data['attempts'] -= 1
                if email_change_data['attempts'] <= 0:
                    del request.session['email_change']
                    messages.error(request, "Превышено число попыток")
                else:
                    request.session['email_change'] = email_change_data
                    messages.error(request, f"Неверный код. Осталось попыток: {email_change_data['attempts']}")
                
                return redirect('change_email')
    
    # Для GET-запросов и после обработки POST
    form = EmailChangeForm(user=request.user)
    return render(request, 'home/change_email.html', {
        'form': form,
        'email_change': email_change_data,
        'remaining_cooldown': int(remaining_cooldown)
    })
@login_required
def change_password(request):
    session_data = request.session.get('password_change', {})
    remaining_cooldown = 0
    form = PasswordChangeWithCodeForm(user=request.user)

    # Расчет времени кулдауна
    if session_data.get('last_sent'):
        try:
            last_sent = datetime.fromisoformat(session_data['last_sent'])
            remaining = (last_sent + timedelta(seconds=60)) - timezone.now()
            remaining_cooldown = max(remaining.total_seconds(), 0)
        except Exception:
            session_data = {}
    ...
    if request.method == "POST":
        if 'send_code' in request.POST:
            ...
            request.session['password_change'] = session_data
            ...
            if form.is_valid():
                # Проверка кода
                if session_data.get('code') != form.cleaned_data['confirmation_code']:
...
from django.db.models import Q, Func, F, TextField, Value
...
class UnicodeLower(Func):
    function = 'LOWER'
    template = '%(function)s(%(expressions)s)'
class CaseInsensitiveReplace(Func):
    function = 'REPLACE'
    template = "%(function)s(%(expressions)s, '', '')"
    output_field = TextField()
...
def dashboard_view(request):
    ...
    if search_query:
        # Нормализация и экранирование запроса
        clean_query = re.sub(r'\s+', ' ', search_query)  # Удаляем лишние пробелы
        clean_query = re.escape(clean_query)  # Экранируем спецсимволы
        
        # Создаем регистронезависимый regex паттерн
        applications = applications.filter(
            title__iregex=f"^{clean_query}|{clean_query}$|.*{clean_query}.*"
        )
    if app_type:
        applications = applications.filter(innovation_type=app_type)
    # Сортировка
    sort_map = {
        'new': '-created_at',
        'old': 'created_at',
    ...
    applications = applications.order_by(sort_map.get(sort, '-created_at'))

    context = {
        'applications': applications,
        'INNOVATION_TYPE_CHOICES': Application.INNOVATION_TYPE_CHOICES,
